...
Last modified: Wed Sep 19 22:29:38 2018
"""
...
for f in files:
    if f == "femb_config_wr_v323_fpgadac_25_4.cfg":
        v323_cfgs = []
        with open(root_path + f, 'r') as cf:
            for cl in cf:
                v323_cfgs.append ( cl.split(",")[0:5])
    if f == "femb_config_wr_v325c_fpgadac_25_4.cfg":
        v325c_cfgs = []
        with open(root_path + f, 'r') as cf:
            for cl in cf:
...
for idir in sorted(dirs):
    if (idir.find("APA") >= 0 ):
        ...
        for cff in sorted(subfiles):
            cffp = cfp + cff
            if (cffp.find("step30") > 0 ) and (cffp.find("CHK_FE_ADC.txt") >=0 ) and (cffp.find("CHK_FE_ADC.txt.swp") <0 ): #for fpga dac
                fembcfgs = []
                with open(cffp, 'r') as cf:
                    for cl in cf:
                        fembcfgs.append(cl) 

                if ( fembcfgs[1].find("step30")>= 0):
...
csvfile =  "/Users/shanshangao/Documents/GitHub/ProtoDUNE_CFG/" + 'ProtoDUNE_SP_FEMBs_Config_fpgadac4_25.csv'
