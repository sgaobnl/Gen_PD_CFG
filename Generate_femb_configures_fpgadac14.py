...
Last modified: Wed Sep 19 22:30:33 2018
"""
...
for f in files:
    if f == "femb_config_wr_v323_fpgadac_14_8.cfg":
        v323_cfgs = []
        with open(root_path + f, 'r') as cf:
            for cl in cf:
                v323_cfgs.append ( cl.split(",")[0:5])
    if f == "femb_config_wr_v325c_fpgadac_14_8.cfg":
        v325c_cfgs = []
        with open(root_path + f, 'r') as cf:
            for cl in cf:
...
for idir in sorted(dirs):
    if (idir.find("APA") >= 0 ):
        ...
        for cff in sorted(subfiles):
            cffp = cfp + cff
            if (cffp.find("step10") > 0 ) and (cffp.find("CHK_FE_ADC.txt") >=0 ) and (cffp.find("CHK_FE_ADC.txt.swp") <0 ): #for fpga dac
                fembcfgs = []
                with open(cffp, 'r') as cf:
                    for cl in cf:
                        fembcfgs.append(cl) 

                if ( fembcfgs[1].find("step10")>= 0):
...
csvfile =  "/Users/shanshangao/Documents/GitHub/ProtoDUNE_CFG/" + 'ProtoDUNE_SP_FEMBs_Config_fpgadac8_14.csv'
